...
def load_prepared_documents(input_path: Path, limit: int = None) -> List[Dict[str, Any]]:
    """
...
def process_batch(
    ...
    """
    logger.info(f"\n{'='*70}")
    logger.info(f"BATCH {batch_num}/{total_batches} - Documents {start_idx+1}-{start_idx+len(batch)}")
    logger.info(f"{'='*70}")
    ...
    for idx, doc in enumerate(batch):
        ...
        try:
            ...
            batch_stats['entities_stored_neo4j'] += result.get('entities_stored_neo4j', 0)
            batch_stats['entities_stored_milvus'] += result.get('entities_stored_milvus', 0)
    ...
    logger.info(f"{'='*70}")
    logger.info(f"Success: {batch_stats['documents_processed']}/{len(batch)} docs ({success_rate:.1f}%)")
    logger.info(f"Entities: {batch_stats['entities_extracted']} extracted, {batch_stats['entities_stored_neo4j']} in Neo4j, {batch_stats['entities_stored_milvus']} in Milvus")
    logger.info(f"Relations: {batch_stats['relations_found']} found, {batch_stats['relations_stored']} stored")
...
def main():
    ...
    logger.info("="*70)
    logger.info(f"Work Order: GPU_CHROMADB_PAOAS_INGESTION.md")
    ...
    for batch_num in range(total_batches):
        ...
        try:
            ...
            overall_stats['entities_stored_neo4j'] += batch_stats['entities_stored_neo4j']
            overall_stats['entities_stored_milvus'] += batch_stats['entities_stored_milvus']
    ...
    logger.info("="*70)
    logger.info(f"Documents: {overall_stats['documents_processed']}/{overall_stats['total_documents']} processed ({overall_stats['documents_failed']} failed)")

    success_rate = (overall_stats['documents_processed'] / overall_stats['total_documents']) * 100
    error_rate = (overall_stats['documents_failed'] / overall_stats['total_documents']) * 100
    ...
    # Gate 1: Functional
    gate1_pass = (
        overall_stats['documents_processed'] >= 10 and
        len(overall_stats['gpu_metrics']) > 0
    )
    logger.info(f"Gate 1 (Functional): {'✓ PASS' if gate1_pass else '✗ FAIL'}")
    logger.info(f"  - Documents processed: {overall_stats['documents_processed']} >= 10")
    logger.info(f"  - GPU detected: {len(overall_stats['gpu_metrics']) > 0}")

    # Gate 2: Integration
    gate2_pass = (
        storage_stats.get('neo4j_entities', 0) > 0 and
        storage_stats.get('milvus_entities', 0) > 0
    )
    ...
    if overall_stats['gpu_metrics']:
        avg_gpu_util = sum(m['gpu_utilization_pct'] for m in overall_stats['gpu_metrics']) / len(overall_stats['gpu_metrics'])
        max_vram = max(m['memory_used_mb'] for m in overall_stats['gpu_metrics'])

    throughput = overall_stats['documents_processed'] / overall_stats['total_time'] if overall_stats['total_time'] > 0 else 0

    gate3_pass = (
        max_vram > 2000 and  # > 2GB VRAM
        throughput > 0.5 and  # > 0.5 docs/sec
        avg_gpu_util > 20  # > 20% avg utilization
    )
    ...
    gate4_pass = (
        error_rate < 5.0 and
        overall_stats['batches_completed'] == total_batches
    )
    logger.info(f"Gate 4 (Stability): {'✓ PASS' if gate4_pass else '✗ FAIL'}")
    logger.info(f"  - Error rate: {error_rate:.1f}% < 5%")
    logger.info(f"  - All batches completed: {overall_stats['batches_completed']}/{total_batches}")
    ...
    logger.info(f"Full log available at: /tmp/paoas_ingestion_full.log")
