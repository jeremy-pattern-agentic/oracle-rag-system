...
def process_batch(
    ...
    for idx, doc in enumerate(batch):
        try:
            ...
            result = pipeline.process_text(text=doc["text"], metadata=doc["metadata"])
...
def main():
    ...
    logger.info("=" * 70)
    logger.info(
        f"Documents processed:    {overall_stats['documents_processed']}/{overall_stats['total_documents']}"
    )
    ...
    logger.info(
        f"Avg time per document:  {overall_stats['total_time'] / overall_stats['documents_processed']:.2f}s"
    )
    logger.info("")
    ...
    if overall_stats["documents_processed"] == overall_stats["total_documents"]:
        ...
        logger.warning(
            f"⚠️  Partial completion: {overall_stats['documents_processed']}/{overall_stats['total_documents']}"
        )
        logger.info(
